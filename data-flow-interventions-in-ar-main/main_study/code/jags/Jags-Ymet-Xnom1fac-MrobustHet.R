...
source("../jags/DBDA2E-utilities.R")
...
genMCMC = function( datFrm , yName="y" , xName="x" ,
  ...
  aGammaShRa = unlist( gammaShRaFromModeSD( mode=sd(y)/2 , sd=2*sd(y) ) ) # make the prior broad on the scale of the data
  ...
  #------------------------------------------------------------------------------
  # THE MODEL.
  # define the model (commented by NB for clarity)
  modelString = " # open quote for setting model specs as a string
  model{
  ...
  writeLines(modelString, con="../jags/TEMPmodel.txt" )
  #------------------------------------------------------------------------------
...
plotMCMC = function( codaSamples , 
  ...
  for ( xidx in 1:length(xlevels) ) {
    ...
    points( rep(xPlotVal,length(yVals))+runif(length(yVals),-0.05,0.05) , 
            yVals , pch=1 , cex=1.5 , col="#E96A5D" )
    ...
    for ( chnIdx in chainSub ) {
      ...
      yt = 0.67*yt/max(yt)
      lines( xPlotVal-yt , ycomb , col="#6497B1" ) 
    }
  }
  if ( !is.null(saveName) ) {
